...
def _build_tower_claude_md(spec: TowerDeploySpec) -> str:
    ...
    lines = [
        ...
        "",
        "1. Wait 30 seconds",
        "2. Check leader status: `atc leader message --project-id <id> --message 'Status update: what have you completed so far and what are you working on now?'`",
        "3. Check progress: `curl -s http://127.0.0.1:8420/api/projects/<id>/leader/progress`",
        "4. If leader is stuck or silent for >2 minutes: resend the goal with `atc leader message`",
        "5. If leader fails 3 times: report to user with a summary and ask how to proceed",
        "6. Otherwise: repeat from step 1 every 60 seconds until all tasks are done",
        "",
