...
def _build_manager_instructions_md(spec: ManagerDeploySpec) -> str:
    ...
    lines = [
        ...
        "",
        "#### Step 1 — Report active / goal accepted",
        "",
        "Before decomposing or doing any other task work, report that you accepted the kickoff:",
        "```",
        (
            f"curl -s -X POST {spec.api_base_url}/api/projects/{spec.project_id}"
            "/leader/report-active \\"
        ),
        '  -H "Content-Type: application/json" \\',
        '  -d \'{"goal_accepted": true, "message": "Kickoff accepted; decomposing now"}\'',
        "```",
        "This is required startup proof for Tower. Do not skip it.",
        "",
        "#### Step 2 — Read the task graph before planning",
        "",
        "Before creating or assigning anything, always inspect the current task graph yourself:",
        "```",
        f"curl -s {spec.api_base_url}/api/projects/{spec.project_id}/task-graphs",
        "```",
        "Do not ask Tower whether tasks exist. If this returns an empty array, explicitly treat",
        "the project as unplanned and bootstrap the task graph from the goal. If it contains",
        "tasks, reconcile them with the goal before adding more.",
        "",
        "#### Step 3 — Decompose the goal into tasks when the graph is empty",
        "",
        ...
        "",
        "#### Step 4 — Spawn Aces for ready tasks",
        "",
        ...
        "",
        "#### Step 5 — Check Ace startup/trust health before nudges",
        "",
        "Hard expectation: every newly spawned managed Ace may stop at a folder trust/startup",
        "prompt before it can run the task. Before resending instructions, sending a nudge,",
        "or treating an acknowledgement/session row as acceptance, check the Ace health:",
        "```",
        (
            f"atc ace health --api {spec.api_base_url} "
            f"--project-id {spec.project_id} --ace-id <ace_session_id>"
        ),
        "```",
        "If health reports a startup/trust blocker, run inspect-first recovery before nudging:",
        "```",
        (
            f"atc ace recover --api {spec.api_base_url} "
            f"--project-id {spec.project_id} --ace-id <ace_session_id> --dry-run"
        ),
        "```",
        "Only after startup blockers are cleared should you resend instructions or wait for",
        "assignment acceptance evidence.",
        "",
        "#### Step 6 — Send work instructions to each Ace",
        "",
        "For each entry in `spawned`, send the detailed task instruction:",
        "```",
        ...
        "```",
        "",
        "#### Step 7 — Monitor Ace/project progress yourself",
        "",
        "```",
        f"curl -s {spec.api_base_url}/api/projects/{spec.project_id}/leader/progress",
        "```",
        "Poll this yourself while supervising Aces. Tower will not poll after handoff;",
        "Tower waits for your explicit completion hook when the project is verified done.",
        "",
        "#### Step 8 — Review and mark tasks done",
        "",
        "Inspect the output files in the repo/staging directory. For each completed task:",
        "```",
        f"curl -s -X POST {spec.api_base_url}/api/projects/{spec.project_id}/leader/task-done \\",
        '  -H "Content-Type: application/json" \\',
        '  -d \'{"task_graph_id": "<id>"}\'',
        "```",
        "",
        "#### Step 9 — Report completion to Tower via hook",
        "",
        "When all tasks are marked done and verified, call the completion hook; do not",
        "wait for Tower to poll project progress:",
        "```",
        f"atc leader report-complete --api {spec.api_base_url} --project-id {spec.project_id} \\",
        '  --summary "What was completed" \\',
        '  --evidence "/absolute/path/or/url"',
        "```",
        "",
        "You own the task graph: create it, maintain it, and drive it to completion.",
        "Every fresh provider session needs a real prompt plus submit/Enter after any startup",
        "or trust prompt is cleared. If you ever see your own prompt waiting after startup,",
        "continue from the goal immediately instead of waiting for Tower.",
...
def _build_tower_instructions_md(spec: TowerDeploySpec) -> str:
    ...
    lines = [
        ...
        "",
        "## Startup Verification and Event-Driven Handoff",
        "",
        "After starting a Leader, you verify startup only; you do NOT become the busy",
        "project session after handoff:",
        "",
        "1. Immediately check Leader health/startup readiness.",
        "2. Resolve provider-neutral startup/trust blockers through recovery commands.",
        "3. Confirm the Leader accepted the goal or began actionable project work.",
        "4. Then STOP polling normal project/task progress. The Leader is now the busy",
        "   project session and owns task graph/Ace monitoring.",
        "5. Wait for the Leader completion hook/event instead of checking repeatedly:",
        "   `atc leader report-complete --project-id <id> --summary '...'`",
        "6. Only inspect again if the Leader reports blocked/failed, its runtime disappears,",
        "   an operator asks for detail, or an explicit recovery threshold is reached.",
        "",
        ...
        "- **Verify startup** — clear startup/trust blockers and confirm Leader acceptance.",
        "- **Handoff** — after Leader acceptance, stop polling project progress; Leader is busy.",
        "- **Report** — surface explicit Leader blockers, failures, and completion hooks",
        "  to the user.",
        "",
        "You never write code directly — always delegate through Leaders.",
        "You never manage individual tasks — that is the Leader's job.",
        "You never use direct Ace/task assignment commands from Tower context unless",
        "the operator explicitly approves audited break-glass access with a reason.",
        ...
        "start Leaders, verify startup/goal acceptance, hand off execution to the Leader,",
        "wait for completion hooks, and report status back to the user. State that you must",
        "not write code, manage individual Ace tasks, poll project progress after handoff,",
    ...
    lines.extend(
        [
            ...
            "### Ace Visibility",
            "```bash",
            "atc ace list --project-id <id>                       # Observe Aces for a project",
            "```",
            "Direct Ace create/health/recover/message/delete and direct task assignment are",
            "blocked from Tower context unless the operator approves audited break-glass",
            "access with `--break-glass-approved --break-glass-reason '...'`.",
            "",
            "### Workflow",
            "",
            ...
            "3. Hard startup expectation: immediately check Leader health before",
            "   using progress=0 or sending a nudge:",
            "   `atc leader health --project-id <id> --summary`",
            "   A managed Leader may show a folder trust/startup prompt after",
            "   the session exists. This is an expected branch, not a surprise.",
            "4. If health reports `runtime_trust_required`,",
            "   `blocked_on_provider_prompt`, or guidance like",
            "   `resolve_startup_trust_prompt_before_nudge`, run:",
            "   `atc leader recover --project-id <id> --dry-run`",
            "   Apply recovery only if the provider-owned plan says the managed",
            "   workspace trust prompt is safe to resolve. Do not send nudges yet.",
            "5. After startup/trust blockers are cleared, verify kickoff proof with:",
            "   `curl -s http://127.0.0.1:8420/api/projects/<id>/leader/progress`",
            "   A new Leader is not truly started until the provider has received",
            "   a prompt and submit/Enter, trust/startup prompts are cleared,",
            "   and the Leader reports goal accepted or creates/reads tasks.",
            "6. If progress shows `prompt_not_submitted`, `kickoff_unverified`,",
            "   or no task graph activity and health shows no startup blocker,",
            "   send ONE short prompt-submit nudge:",
            "   `atc leader message --project-id <id> --message "
            "'Please read your goal, inspect the task graph, and continue.'`",
            "7. After Leader acceptance/actionable work is verified, hand off execution.",
            "   Do not poll normal project progress to discover completion; the Leader is",
            "   the busy session and must notify Tower through:",
            "   `atc leader report-complete --project-id <id> --summary '...'`",
            "",
            ...
            "Expect a prompt plus Enter/submit after every managed provider",
            "session starts or resumes;",
            "do not assume a pane/session row means the Leader has begun executing.",
            "",
        ]
    )
