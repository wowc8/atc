...
async def test_spawn_leader_reuses_persisted_operation(db_conn) -> None:
    ...
    response = await service.spawn_leader(
        SpawnLeaderRequest(project_id=project.id, goal="Ship it", idempotency_key="goal-1")
    )
    assert response.operation_id == "goal-1"
    assert response.request_status == "queued"
    assert response.delivery_state == "queued"
    assert tower.calls == []
...
async def test_spawn_ace_reuses_persisted_operation(db_conn) -> None:
    ...
    response = await service.spawn_ace(
        SpawnAceRequest(project_id=project.id, instruction="Go", idempotency_key="assign-1")
    )
...
async def test_idempotency_conflict_across_operation_types(db_conn) -> None:
    ...
    with pytest.raises(OrchestrationException) as exc:
        await service.spawn_ace(
            SpawnAceRequest(project_id=project.id, instruction="Go", idempotency_key="dup-1")
        )
