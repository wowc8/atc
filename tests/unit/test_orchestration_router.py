...
def _delivery_result() -> RuntimeDeliveryResult:
...
async def test_spawn_orchestration_ace(app_and_db) -> None:
    ...
    with (
        patch(
            ...
            new=AsyncMock(return_value="session-123"),
        ),
        patch(
        ...
        patch(
            "atc.orchestration.service.OrchestrationService.get_session",
    ...
    assert body["request_status"] == "submitted"
    assert body["delivery_state"] == "submitted"
